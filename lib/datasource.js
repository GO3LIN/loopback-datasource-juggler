...
DataSource.prototype.discoverManyToManyRelations = function(modelName, options, cb) {
  this.freeze();
  ...
  if (this.connector.discoverManyToManyRelations) {
    this.connector.discoverManyToManyRelations(modelName, options, cb);
  } else if (cb) {
    process.nextTick(cb);
  }
  return cb.promise;
...
DataSource.prototype.discoverSchemas = function(modelName, options, cb) {
  ...
  if (followingRelations) {
    ...
    tasks.push(this.discoverManyToManyRelations.bind(this, modelName, options));
  }
  ...
  async.parallel(tasks, function(err, results) {
    ...
    columns.forEach(function(item) {
      ...
      if (pks[item.columnName]) {
        schema.properties[propName].id = pks[item.columnName].key;
        // Handle identity PKs
        if(pks[item.columnName].isIdentity == 1){
          console.log('identity detected');
          schema.properties[propName].generated = true;
          schema.properties[propName].required = false;
        } else {
          schema.properties[propName].generated = false;
        }
      }
    ...
    if (followingRelations) {
      ...
      // HANDLING hasOne + hasMany
      var externalFKs= results[3] || [];
      externalFKs.forEach(function(efk){
        var propName = nameMapper('column', efk.fkTableName);
        var type = "hasMany";
        // IF the foreign key is also a primary key (not composite)
        if(efk.fkSeq === 1 && efk.pkComposite === 1){
          type="hasOne";
        }
        schema.options.relations[propName] = {
            model: nameMapper('table', efk.fkTableName),
            type: type,
            foreignKey: nameMapper('column', efk.fkColumnName)
        };
        
      });

      var manyToManyFks = results[4] || [];
      manyToManyFks.forEach(function(mmFk){
        var propName = nameMapper('column', mmFk.fkTableName);
        
        // If there's already a 1-1 relation between the two models just pass
        // if(schema.options.relations[propName] && schema.options.relations[propName].type == 'hasOne'){
        //   console.log(propName+' is already a hasOne');
        //   return;
        // }
        // else {

        // Make hasMany relations in plurial to avoid conflicting with hasOne relations.
        schema.options.relations[propName+'s'] = {
          model: nameMapper('table', mmFk.fkTableName),
          type: "hasMany",
          foreignKey: nameMapper('column', mmFk.fkColumnName),
          through: nameMapper('table', mmFk.throughTableName),
          keyThrough: nameMapper('column', mmFk.throughColumnName)
        };
        // }
      });
